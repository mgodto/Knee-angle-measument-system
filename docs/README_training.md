# Knee X-ray Keypoint Training and Evaluation
...
The retraining runners use `.venv-training/bin/python` by default. Create that
environment from the repository root before the first run:

```bash
python3 -m venv .venv-training
source .venv-training/bin/activate
python -m pip install --upgrade pip
python -m pip install -r requirements/requirements-training.txt
```

Set `KNEE_RETRAINING_PYTHON_BIN` when a different compatible Python environment
must be used.

## Commands

```bash
python -m knee_xray.data.build_dataset_manifest --output outputs/knee_dataset_manifest.csv
python -m knee_xray.data.validate_knee_dataset --manifest outputs/knee_dataset_manifest.csv
python -m knee_xray.training.train_keypoint_baseline --manifest outputs/knee_dataset_manifest.csv
```
...
```bash
python -m knee_xray.data.build_dataset_manifest \
...
Add `--clean` to `python -m knee_xray.data.organize_implant_dataset` when rebuilding the organized
...
```bash
bash scripts/training/run_retraining_single_leg_v2_20260803.sh
```
...
```bash
bash scripts/training/run_retraining_single_leg_v3_20260811.sh --preflight-only
```
...
```bash
bash scripts/training/run_retraining_single_leg_v3_20260811.sh
```
...
```bash
python -m knee_xray.training.train_keypoint_baseline \
  --dataset-dir "images/annotation_dataset_by_implant/未加入人工關節" \
...
```bash
python -m knee_xray.training.visualize_keypoint_predictions \
  --dataset-dir "images/annotation_dataset_by_implant/未加入人工關節" \
...
```bash
python -m knee_xray.training.prepare_retraining_manifests \
...
python -m knee_xray.training.evaluate_keypoint_checkpoint \
...
```bash
python -m knee_xray.data.import_annotation_batch \
