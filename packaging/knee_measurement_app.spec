...
entry_script = (
    project_root / "knee_measurement_app_windows.py"
    if sys.platform == "win32"
    else project_root / "knee_xray" / "ui" / "knee_measurement_app.py"
)
config_path = project_root / "config" / "knee_measurement_app.json"
...
if not entry_script.is_file():
    raise FileNotFoundError(
        ...
        "On Windows, run knee_xray.release.generate_windows_english_entrypoint first."
    )
...
hiddenimports = ["knee_xray.ml.knee_keypoint_model"]
datas = [
    (str(config_path), "."),
