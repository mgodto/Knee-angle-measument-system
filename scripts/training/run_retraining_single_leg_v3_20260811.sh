...
run_fold() {
  ...
  if training_complete fold "$manifest" "$train_dir" "$fold_epochs" "$fold" "$version" "$scope"; then
    ...
    "$python_bin" -m knee_xray.training.train_keypoint_baseline \
      --manifest "$manifest" \
  ...
  if evaluation_complete "$manifest" "$train_dir/best.pt" "$evaluation_dir" "$fold"; then
    ...
    "$python_bin" -m knee_xray.training.evaluate_keypoint_checkpoint \
...
run_model() {
  ...
  if cv_complete "$manifest" "$cv_dir"; then
    ...
    "$python_bin" -m knee_xray.training.summarize_keypoint_cv \
      "${evaluation_inputs[@]}" \
      --output-dir "$cv_dir"
...
require_file knee_xray/training/train_keypoint_baseline.py
require_file knee_xray/training/evaluate_keypoint_checkpoint.py
require_file knee_xray/training/summarize_keypoint_cv.py
require_file knee_xray/data/validate_knee_dataset.py
...
"$python_bin" -m knee_xray.data.validate_knee_dataset --manifest "$manifest_root/bone_confirmed.csv"
"$python_bin" -m knee_xray.data.validate_knee_dataset --manifest "$manifest_root/tka.csv"
"$python_bin" - <<'PY'
import torch
