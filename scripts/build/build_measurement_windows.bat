...
set "DELIVERY_DIR=deliverables\measurement\windows"
set "DELIVERY_STAGE=build\deliverable-stage\measurement\windows"
set "CANDIDATE_ZIP=%DELIVERY_STAGE%\%RELEASE_ROOT%.zip"
set "RELEASE_ZIP=%DELIVERY_DIR%\%RELEASE_ROOT%.zip"
set "RELEASE_STAGE_ROOT=build\release-stage-measurement"
...
if errorlevel 1 exit /b 1
python -m knee_xray.release.generate_windows_english_entrypoint --source knee_xray\ui\knee_measurement_app.py --output "%WINDOWS_ENTRYPOINT%"
if errorlevel 1 exit /b 1
...
if errorlevel 1 exit /b 1
python -m knee_xray.release.create_release_smoke_fixture "%SMOKE_IMAGE%"
if errorlevel 1 exit /b 1
...
if exist "%PYINSTALLER_DIST%\KneeXrayMeasurement" rmdir /s /q "%PYINSTALLER_DIST%\KneeXrayMeasurement"
...
pyinstaller --noconfirm --clean --distpath "%PYINSTALLER_DIST%" --workpath "%PYINSTALLER_WORK%" packaging\knee_measurement_app.spec
if errorlevel 1 exit /b 1
"%PYINSTALLER_DIST%\KneeXrayMeasurement\KneeXrayMeasurement.exe" --validate-models --expected-model-version bone=%EXPECTED_BONE_VERSION% --expected-model-version tka=%EXPECTED_TKA_VERSION% --expected-model-version mixed=%EXPECTED_MIXED_VERSION%
if errorlevel 1 exit /b 1

mkdir "%RELEASE_STAGE%"
if errorlevel 1 exit /b 1
xcopy /E /I /Y "%PYINSTALLER_DIST%\KneeXrayMeasurement\*" "%RELEASE_STAGE%\" >nul
if errorlevel 1 exit /b 1
copy /Y docs\README_DOCTOR_EN.txt "%RELEASE_STAGE%\README_DOCTOR_EN.txt" >nul
if errorlevel 1 exit /b 1
...
powershell -NoProfile -Command "Compress-Archive -CompressionLevel Optimal -Path '%RELEASE_STAGE%' -DestinationPath '%CANDIDATE_ZIP%'"
if errorlevel 1 exit /b 1
python -m knee_xray.release.audit_measurement_archive "%CANDIDATE_ZIP%" --platform windows --expected-root "%RELEASE_ROOT%" --expected-model "bone.pt=%EXPECTED_BONE_SHA%" --expected-model "tka.pt=%EXPECTED_TKA_SHA%" --expected-model "mixed.pt=%EXPECTED_MIXED_SHA%" --expected-app-version "%APP_VERSION%"
if errorlevel 1 exit /b 1
move /Y "%CANDIDATE_ZIP%" "%RELEASE_ZIP%" >nul
if errorlevel 1 exit /b 1
python -m knee_xray.release.retain_deliverables measurement windows --repo-root "%CD%" --current "%RELEASE_ZIP%"
if errorlevel 1 exit /b 1
