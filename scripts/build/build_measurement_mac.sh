...
delivery_dir="$PWD/deliverables/measurement/macos"
delivery_stage="$PWD/build/deliverable-stage/measurement/macos"
candidate_zip="$delivery_stage/${release_root}.zip"
release_zip="$delivery_dir/${release_root}.zip"
...
python -c 'from knee_xray.ui.knee_measurement_app import APP_RELEASE_CHANNEL, APP_VERSION; assert APP_VERSION == "0.6.0", APP_VERSION; assert APP_RELEASE_CHANNEL == "INTERNAL RESEARCH CANDIDATE - NOT FOR CLINICAL USE", APP_RELEASE_CHANNEL'
...
rm -rf "$pyinstaller_dist/KneeXrayMeasurement" "$pyinstaller_dist/KneeXrayMeasurement.app" \
  "$pyinstaller_work/knee_measurement_app"
rm -f "$candidate_zip"
mkdir -p "$delivery_dir" "$delivery_stage"
pyinstaller --noconfirm --clean \
  --distpath "$pyinstaller_dist" \
  --workpath "$pyinstaller_work" \
  packaging/knee_measurement_app.spec

mkdir -p "$package_stage/$release_root"
rsync -a --exclude '.DS_Store' --exclude '._*' --exclude '__MACOSX' \
  "$pyinstaller_dist/KneeXrayMeasurement.app" "$package_stage/$release_root/"
cp docs/README_DOCTOR_JA.txt "$package_stage/$release_root/README_DOCTOR_JA.txt"

(
  cd "$package_stage"
  COPYFILE_DISABLE=1 zip -qry -y -X "$candidate_zip" "$release_root"
)
python -m knee_xray.release.audit_measurement_archive "$candidate_zip" \
  --platform macos \
...
unzip -q "$candidate_zip" -d "$extracted_stage"
...
python -m knee_xray.release.retain_deliverables measurement macos \
